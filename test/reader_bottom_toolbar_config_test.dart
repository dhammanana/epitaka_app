...
void main() {
  ...
  testWidgets('default config renders every action in natural order', (
    ...
    final contentsY = tester.getTopLeft(find.byIcon(Icons.format_list_bulleted)).dx;
  ...
  testWidgets('disabled items are hidden, enabled order is preserved', (
    ...
    final items = [
      ...
      ...defaultToolbarItems().where(
        (i) => i.id != ToolbarBuiltins.jump && i.id != ToolbarBuiltins.search,
      ),
    ].map((i) {
      if (i.id == ToolbarBuiltins.bookmark) return i.copyWith(enabled: false);
      return i;
    }).toList();
  ...
  testWidgets('flat status bar skips actions it does not wire up', (
    ...
    // Flat mode only wires jump/display/listen/bookmark (like the desktop
    // status bar) — contents/outline/search/dictionary/annotations must be
    // skipped even though they're enabled in the config.
  ...
  testWidgets('all actions disabled renders nothing', (tester) async {
    final items = [
      for (final id in ToolbarBuiltins.defaults) ToolbarItem(id: id, enabled: false),
    ];
    await tester.pumpWidget(wrap(toolbar(items: items)));
    await tester.pump();
